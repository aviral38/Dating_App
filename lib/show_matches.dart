...
class Show_matches extends StatefulWidget {
  final Usersclass user;
  Show_matches({this.user});

  @override
  _Show_matchesState createState() => _Show_matchesState();
}

class _Show_matchesState extends State<Show_matches> {
  FirebaseFirestore _firestore = FirebaseFirestore.instance;

  Future<List<Usersclass>> getMatchedUsers() async{
    List<String> matchList=await getMatchList(widget.user.uid);
    print(matchList.toString());

    List<Usersclass> matched_user=[];

    for(int i=0;i<matchList.length;i++)
      {
        Usersclass user=await getCurrentUser(matchList[i]);
        matched_user.add(user);
      }
    return matched_user;
  }
  ...
  Future<Usersclass> getCurrentUser(String userid) async
  {
    ...
    print('hrr'+currentUser.photo);
    return currentUser;
  }
  Future Unmatch({currentUserId,selectedUserId}) async{
    ...
    await _firestore.collection('users').doc(currentUserId).collection('selectedList').doc(selectedUserId).delete();
    await _firestore.collection('users').doc(selectedUserId).collection('selectedList').doc(currentUserId).delete();
  }
  Future getMatchList(String userid) async {
      ...
      body: FutureBuilder(
        ...
        builder: (context,snapshot){
          if(snapshot.data==null)
            {
              return Center(child: Text('Start Swiping to get Matches'),);
            }
          else{
            List<Usersclass> matchList = snapshot.data;
            if(matchList.length==0)
              {
                return Center(child:Text('Start Swiping to get Matches'));
              }
            return ListView.builder(scrollDirection: Axis.vertical,itemCount: matchList.length,
                itemBuilder: (BuildContext context,int index){
              return profileWidget( padding: size.height*0.035,
                photoHeight: size.height*0.650,
                photo: matchList[index].photo,
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(matchList[index].name.toUpperCase(),
                    style: TextStyle(
                      fontFamily: 'Lato',
                      fontSize: 40,
                      color:  Colors.white,
                  ...
                  Text((DateTime.now().year-matchList[index].age.toDate().year).toString(),
                    style: TextStyle(
                      fontFamily: 'Lato',
                      fontSize: 40,
                      color:  Colors.white,
                  ...
                  Row(
                    children: [
                      FlatButton(onPressed: (){
                        //print(matchList[index].uid);
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => Messaging_page(currentuser: widget.user,selectedUser: matchList[index],),
                            //builder: (context) => messages(userid: uid),
                          ),
                        );
                      }, child: Icon(Icons.chat,color: Colors.white,size: 50,)),
                      FlatButton(color:Color(0xffF24E86),
                          onPressed: ()async{
                        await Unmatch(currentUserId: widget.user.uid,selectedUserId: matchList[index].uid);
                        setState(() {
